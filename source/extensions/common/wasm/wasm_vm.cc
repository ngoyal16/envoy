...
bool EnvoyWasmVmIntegration::getNullVmFunction(absl::string_view function_name, bool returns_word,
  ...
  if (function_name == "envoy_on_resolve_dns" && returns_word == false &&
    ...
    *reinterpret_cast<proxy_wasm::WasmCallVoid<3>*>(ptr_to_function_return) = [plugin](
        ContextBase* context, Word context_id, Word token, Word result_size) {
      ...
      if (auto root = context_base->asRoot()) {
        static_cast<proxy_wasm::null_plugin::EnvoyRootContext*>(root)->onResolveDns(token,
                                                                                    result_size);
      }
    };
    return true;
    ...
    *reinterpret_cast<proxy_wasm::WasmCallVoid<2>*>(ptr_to_function_return) = [plugin](
        ContextBase* context, Word context_id, Word result_size) {
      proxy_wasm::SaveRestoreContext saved_context(context);
      // Need to add a new API header available to both .wasm and null vm targets.
      auto context_base = plugin->getContextBase(context_id);
      if (auto root = context_base->asRoot()) {
